...
class _MissionDetailScreenState extends ConsumerState<MissionDetailScreen> {
  ...
  @override
  void didUpdateWidget(MissionDetailScreen oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.runId != widget.runId) _load();
  }

  void _load() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ref.read(missionControllerProvider.notifier).open(widget.runId);
      }
    });
  }
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  ...
                  TextButton.icon(
                    onPressed: () =>
                        context.push('/tasks?run_id=${widget.runId}'),
                    icon: const Icon(Icons.checklist),
                    label: const Text('Tasks'),
                  ),
                  TextButton.icon(
                    onPressed: () =>
                        context.push('/artifacts?run_id=${widget.runId}'),
